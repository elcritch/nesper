...
uint8 ESP_PARTITION_MAGIC = 0x50AA;
uint8 ESP_PARTITION_MAGIC_MD5 = 0xEBEB;
uint8 PART_TYPE_APP = 0x00;
uint8 PART_SUBTYPE_FACTORY =  0x00;
uint8 PART_SUBTYPE_OTA_FLAG = 0x10;
uint8 PART_SUBTYPE_OTA_MASK = 0x0f;
uint8 PART_SUBTYPE_TEST =     0x20;
uint8 PART_TYPE_DATA = 0x01;
uint8 PART_SUBTYPE_DATA_OTA = 0x00;
uint8 PART_SUBTYPE_DATA_RF =  0x01;
uint8 PART_SUBTYPE_DATA_WIFI = 0x02;
uint8 PART_SUBTYPE_DATA_NVS_KEYS = 0x04;
uint8 PART_SUBTYPE_DATA_EFUSE_EM = 0x05;
uint8 PART_TYPE_END = 0xff;
uint8 PART_SUBTYPE_END = 0xff;
uint8 PART_FLAG_ENCRYPTED = (1<<0);

/* Pre-partition table fixed flash offsets */
uint8 ESP_BOOTLOADER_DIGEST_OFFSET = 0x0;
uint8 ESP_BOOTLOADER_OFFSET = 0x1000; /* Offset of bootloader image. Has matching value in bootloader KConfig.projbuild file. */
uint8 ESP_PARTITION_TABLE_OFFSET = CONFIG_PARTITION_TABLE_OFFSET; /* Offset of partition table. Backwards-compatible name.*/
uint8 ESP_PARTITION_TABLE_MAX_LEN = 0xC00; /* Maximum length of partition table data */
uint8 ESP_PARTITION_TABLE_MAX_ENTRIES = (ESP_PARTITION_TABLE_MAX_LEN / sizeof(esp_partition_info_t)); /* Maximum length of partition table data, including terminating entry */
...
typedef struct {
    ...
    uint8  seq_label[20];
    uint32 ota_state;
    uint32 crc; /* CRC32 of ota_seq field only */
