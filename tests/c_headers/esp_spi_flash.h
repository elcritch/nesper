...
csize_t SPI_FLASH_SEC_SIZE = 4096;    /**< SPI Flash sector size */

csize_t SPI_FLASH_MMU_PAGE_SIZE = 0x10000; /**< Flash cache MMU mapping page size */
 ...
 */
typedef uint32 spi_flash_mmap_handle_t;
 ...
 */
uint32 spi_flash_mmap_get_free_pages(spi_flash_mmap_memory_t memory);


uint32 SPI_FLASH_CACHE2PHYS_FAIL = UINT32_MAX; /*<! Result from spi_flash_cache2phys() if flash cache address is invalid */
 ...
 */
typedef struct {
    uint32 count;     // number of times operation was executed
    uint32 time;      // total time taken, in microseconds
