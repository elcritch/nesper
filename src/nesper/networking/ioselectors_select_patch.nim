...
when defined(windows):
...
elif defined(nimIoselectorEventfd):
  proc eventfd(fd: cint, flags: cint): cint {.importc: "eventfd", header: platformHeaders, inline.}
  ...
  proc trigger*(ev: SelectEvent) =
    var data: uint64 = 1
    if posix.write(ev.rsock.cint, addr data, sizeof(uint64)) == -1:
      raiseIOSelectorsError(osLastError())

  proc close*(ev: SelectEvent) =
    let res = posix.close(ev.rsock.cint)
    deallocShared(cast[pointer](ev))
    if res != 0:
      raiseIOSelectorsError(osLastError())
